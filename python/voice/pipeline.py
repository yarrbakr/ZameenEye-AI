...
async def run(phone: str, text: str) -> str | None:
    ...
    # `text` (the transcript) is logged for now; the advisory is driven by the
    # land's live hazard state.
    # (Future: use `text` for intent / which-land selection.)
    land_id = resolve_land_id(phone)
    if not land_id:
    ...
    advisory = await asyncio.to_thread(generate_advisory, payload)
    if advisory is None:
